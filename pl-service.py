...
class PLService(object):
    ...
    def active(self, asn=None):
        ...
        if not asn:
            ...
            sql = 'SELECT vp, asn FROM  vps WHERE active=1 AND asn=%s AND fails<%s' % (
                str(asn), str(self.__common["consecutivefails"]))
    ...
    def submit(self, vp, target):
        ...
        # submit
        sql = "INSERT INTO traceroutes (status,vp,target, errors,json,raw) VALUES ('%s','%s','%s','%s','%s','%s')" % (
    ...
    def run(self):
        ...
        # server = MultithreadedXMLRPCServer(
        #    (self.__common["querier.binding.host"], int(self.__common["querier.binding.port"])),
        #    requestHandler=SecuredHandler, logRequests=True, auth_map=self.auth_map)
        ...
        server = SimpleXMLRPCServer((self.__common["querier.binding.host"], int(self.__common["querier.binding.port"])), requestHandler=RequestHandler, allow_none=True)
